 ...
 * {@code @create} 2022/9/23 19:58 encrypt-decrypt CSDNLab3<p>
 * <p>
 * <br>
 */
import java.io.*;
...
public class CSDNLab3 {
    static byte[] bytekey;

    private CSDNLab3(String strKey) {
        this.bytekey = strKey.getBytes();
    }

    private static CSDNLab3 instance;

    //单利模式，可以直接访问该类，不需要实例化该类的对象。保证一个类仅有一个实例，并提供一个访问它的全局访问点。
    //懒汉式，线程安全，synchronized实现多线程安全
    public static synchronized CSDNLab3 getInstance(String strKey) {
        if (instance == null) {
            instance = new CSDNLab3(strKey);
        }
        return instance;
    }
    ...
    public static void main(String[] args) throws IOException {
        String key = "abcdABCD";
        CSDNLab3 desUtil = CSDNLab3.getInstance(key);//秘钥变成二进制
        try {
